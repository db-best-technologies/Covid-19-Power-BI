...
$DebugOptions = Set-DebugOptions -WriteFilesToTemp $true -TempPath "C:\Temp\Covid-Temp-Files" -DeleteTempFilesAtStart $false -UpdateLocalFiles $true -AppendDebugData $false -Workaround $false -ForceDownload $true -LoadFromWorkingFiles $false
$DebugOptions 
...
$URLs = @{
    ...
    GetRawLocations         = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/UID_ISO_FIPS_LookUp_Table.csv"
    GetRawZipCodes          = "https://raw.githubusercontent.com/db-best-technologies/Covid-19-Power-BI/master/Data-Files/dimUS_zip_codes_states.csv"
}
$ColumnHeaders = @{
...
if ( $null -ne $FIPS_WR -and $null -ne $FIPS_WR.Content ) {
    ...
    $UID_ISO_FIPS_LookupCSV = Import-Csv -Path $FIPSFilePath | Sort-Object  Combined_Key
...
$CITY_WR = Invoke-WebRequest -Uri $URLs.GetRawZipCodes
...
if ( $null -ne $CITY_WR -and $null -ne $CITY_WR.Content ) {
    ...
    $FileName = $URLs.GetRawZipCodes | Split-Path -Leaf 
    $ZipFilePath = ($GitLocalRoot, $DataDir, $FileName -join "\")
    $CITY_WR.Content | Out-File -FilePath  $ZipFilePath
    $CITY_CSV = Import-Csv -Path $ZipFilePath | Sort-Object  state
    ...
    foreach ( $Zip in $CITY_CSV ) {
        ...
        if ( $null -ne $CountyNameFromCity.$KeyValue ) { continue }
...
if ( $DebugOptions.LoadFromWorkingFiles ) {
...
else <# load data from the SourceWebSite as needed #> {
    ...
    if ( $DebugOptions.ForceDownload = $false ) <# Do a normal process of downloading only the latest files #> { 
        ...
        if ( $null -ne $WebRequest -and $null -ne $WebRequest.Content -and -not $DebugOptions.ForceDownload ) {
            ...
            if ( $null -ne $FilesInfo ) {
                ...
                if ( $FilesInfo[0].FileNumber.Length -gt 0 ) {
                    ...
                    if ( $Mapping.'Province or State' -like "*, *" ) {
                        ...
                        if ( $StateNameFromCode.$StateCode.Province_State.Length -eq 0 ) {
                            if ( $StateCode -eq "D.C." ) {
                                ...
                                $FIPS = $Combined_Key.'District of Columbia, US'
...
if ( $UnpivotedRows.Count -gt 0 ) {
    $UnpivotedRowsFileName = $GitLocalRoot, "\", $DataDir, "\", "CSSEGISandData-COVID-19-Aggregation-Daily-Reports.csv" -join "" 
    $UnpivotedRows | Export-Csv -Path $UnpivotedRowsFileName -NoTypeInformation
}

$FullDataRow | Export-Csv -Path "C:\Users\Bill\OneDrive\Bill\Documents\My GitLab\Covid-19-Power-BI\Working Files\CSSEGISandData-COVID-19-FullDataRow-Daily-Reports.csv" -NoTypeInformation
...
$Columns = $Columns = $FilesInfo[0].psobject.Properties.Name
$FilesInfo | Select-Object -Property $Columns[0..4] | Export-Csv -Path "C:\Users\Bill\OneDrive\Bill\Documents\My GitLab\Covid-19-Power-BI\Data-Files\CSSEGISandData-COVID-19-FullDataRow-SourceFiles.csv" -NoTypeInformation
